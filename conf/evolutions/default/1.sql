...
create table tag_product (
  tag_primary_key                bigint not null,
  product_primary_key            bigint not null,
  constraint pk_tag_product primary key (tag_primary_key, product_primary_key))
;
...
alter table tag_product add constraint fk_tag_product_tag_01 foreign key (tag_primary_key) references tag (primary_key) on delete restrict on update restrict;

alter table tag_product add constraint fk_tag_product_product_02 foreign key (product_primary_key) references product (primary_key) on delete restrict on update restrict;
